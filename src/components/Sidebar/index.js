...
export const Sidebar = props => {
  ...
  const toggleSaveMap = force => {
    setSaveMapModalIsVisible(
      typeof force !== 'undefined' ? force : !saveMapModalIsVisible,
    );
  };
  ...
  return (
    <Sider collapsible collapsed={collapsed} onCollapse={setCollapsed}>
      ...
      >
        <Menu.Item
          data-cy-sidebar-menu-item-new-map
          key="1"
          onClick={() => goToNew(1)}
        >
          <Icon type="file" />
        ...
        {props.maps.map((fm, idx) => (
          <Menu.Item
            ...
            key={idx + 3}
            onClick={() => goToMap(fm.mapSaveNameFormatted, idx + 3)}
          >
            <Icon type="copy" />
            <span>{fm.name}</span>
          </Menu.Item>
        ...
        <Menu.Item
          data-cy-sidebar-menu-item-remove-map
          key="1000"
          onClick={() => triggerDeleteMap()}
        >
          <Icon type="delete" />
